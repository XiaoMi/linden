...
public class TermQueryConstructor extends QueryConstructor {
  ...
  protected Query construct(LindenQuery lindenQuery, LindenConfig config) throws IOException {
    if (lindenQuery.isSetTermQuery()) {
      LindenTermQuery lindenTermQuery = lindenQuery.getTermQuery();
      return new TermQuery(new Term(lindenTermQuery.getTerm().getField(), lindenTermQuery.getTerm().getValue()));
    }
    return null;
  }
